...
- ✅ **M4b fechado:** Pixel antigo (`1202105157078230`) estava preso a uma conta de anúncios pessoal fora do portfólio — nunca liberava o botão de gerar token. Criado Pixel novo `1362777799250881` dentro do portfólio "100pilas". Atualizado em `index.html`, `product.html`, `modules/capi.js`. Token gerado e salvo no painel (campo novo em Ad Center, nunca ecoa o valor de volta).
- ✅ **Custo/margem e Marketplace Hub (M10):** carrinho de custo por pacote + Mercado Livre publicando produto real ficaram prontos e testados em 11/07 (ver seção abaixo). Descoberto nesta sessão que **Shopee e Amazon já têm módulo próprio no código** (`modules/shopee.js`, `modules/amazon.js`) e um novo item de menu "Marketplaces" no painel — construído por outra sessão em paralelo, ainda não auditado a fundo por mim.
- ✅ **Bug crítico de infra corrigido:** `deploy.sh` (o script do auto-deploy) tinha perdido a permissão de execução no servidor (ficou como arquivo comum, não executável). O cron rodava a cada 2 minutos e falhava silenciosamente com "Permission denied" — **nada de novo entrava no ar havia um tempo**, mesmo com os pushes certos no `main`. Corrigido no servidor (`chmod +x`) e no Git (modo `755` commitado, pra não voltar a perder o bit em um clone novo).
- 🔍 **Carrinhos abandonados:** adicionado botão de excluir (individual e "Limpar todos") — usado pra tirar os pedidos/carrinhos de teste do painel.
- ⚠️ **Atenção:** confirmar com quem mexeu em paralelo (outra sessão) o que é o kit "Forpack" (`forpack/`) que apareceu no repositório — não faz parte do ecossistema TopFood, parece ser outro site/cliente hospedado na mesma VPS.
...
| M4b | Rastreamento server-side (CAPI) | Rastreio à prova de bloqueador | ✅ Pronto | Token gerado e salvo no painel (18/07). Pixel migrado pra um novo dentro do portfólio certo (`1362777799250881`) — o antigo estava preso a uma conta pessoal e nunca liberava o token |
...
### Wellington
- [x] ~~Token da Conversions API da Meta~~ → **feito (18/07)**: M4b concluído, ver seção de atualização
