    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        ...
        <tr><td style="padding:28px 32px">
          ...
          <a href="https://wa.me/5511978332442" style="display:inline-block;margin-top:6px;background:#25d366;color:#fff;font-weight:700;font-size:14px;padding:10px 22px;border-radius:8px;text-decoration:none">💬 Falar no WhatsApp</a>
        </td></tr>
        <!-- FOOTER -->
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        ...
        <tr><td style="padding:28px 32px">
          ...
          <a href="https://wa.me/5511978332442" style="display:inline-block;margin-top:16px;background:#25d366;color:#fff;font-weight:700;font-size:14px;padding:10px 22px;border-radius:8px;text-decoration:none">💬 Falar no WhatsApp</a>
        </td></tr>
...
app.get('/api/settings', (req, res) => {
  ...
  res.json({
    ...
    whatsapp:          s.whatsapp          || '5511978332442',
...
<tr><td style="background:#111;padding:24px 40px;text-align:center">
  <p style="color:#9CA3AF;font-size:12px;margin:0 0 6px">TopFood Embalagens &mdash; (11) 97833-2442</p>
...
// As paginas do site ja tem o numero novo no codigo, mas o WhatsApp tambem
// fica salvo em settings (painel > Configuracoes), e dali saem o catalogo PDF
// e o /api/settings. Esta migracao troca os numeros antigos pelo novo no boot,
// pra nao sobrar telefone velho em canto nenhum.
//
// So mexe se o valor salvo for um dos numeros antigos conhecidos — se o
// Wellington cadastrar outro numero no painel, ele fica de pe.
(function atualizarTelefone() {
  const NOVO     = '5511978332442';
  const ANTIGOS  = ['5511988856367', '11988856367', '5511947073962', '11947073962'];
  try {
    const s     = readData('settings.json') || {};
    const atual = String(s.whatsapp || '').replace(/\D/g, '');
    if (!atual || ANTIGOS.includes(atual)) {
      if (atual === NOVO) return;
      s.whatsapp = NOVO;
      writeData('settings.json', s);
      console.log('📱 WhatsApp do site atualizado: ' + (atual || '(vazio)') + ' → ' + NOVO);
    }
  } catch (e) { console.error('[telefone] erro:', e.message); }
})();
